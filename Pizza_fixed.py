...
class sweet_hours:
    ...
    def save_order_to_db(sf, city, locality, building, house, landmark):
        items_str = ', '.join([f"{item[0]} x{item[1]}" for item in sf.cartlist])
        name = getattr(sf, "real_name","")
        email = getattr(sf, "email", "")
        mobile = getattr(sf, "mobile", "")
        ...
        cursor.execute("""
            INSERT INTO orders (
                customer_name, mobile, email, city, locality, building, house_no, landmark,
                items, total_price
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (name, mobile, email, city, locality, building, house, landmark, items_str, sf.amount))
        conn.commit()
        conn.close()

        messagebox.showinfo("Order", "Order successfully placed!\nTap on 'Next' to continue.")
        sf.next_button = Button(
                sf.add_frame2,
                text="Next ➡️",
                command=lambda: sf.thankyou(sf.x),
                fg="white",
                bg="green",
                font=("default", 18),
                bd=5,
        ...
        sf.next_button.place(x=950, y=510)


    
    def show_admin_panel():
        admin = Toplevel()
        admin.title("Admin Panel - Order History")
        admin.geometry("1000x500")
        cols = ("ID", "Name", "Mobile", "Email", "City", "Locality", "Building", "House", "Landmark", "Items", "Price", "Time")
...
#------ login page 2------
    def Login(sf):
        ...
        sf.admin_btn = Button(sf.scr, text="Orders", command=sweet_hours.show_admin_panel, bg="#0b1335", cursor="hand2", bd=4, fg="white", font=("cooper black", 16))

        sf.admin_btn.place(x=10, y=65)
...
#--  login database page 3------
    '''def login_database(sf):
        ...
        if list(x)[0][0]==0:
            ...
            sf.username = sf.credlog[0]
            messagebox.showinfo("Login", f"You have Successfully Logged In\nWelcome {sf.username}")
           # messagebox.showinfo("Login","You have Successfully Log In\nWelcome to the sweet hours")            
            sf.delivery()'''
    
    def login_database(sf):
        sf.credlog = sf.resultlog()  # (username, password)
        sf.con = connect("sweet.db")
        sf.cursor = sf.con.cursor()
    ...
    # Check if user exists
        ...
        # ✅ Fetch real name, email, and mobile
            ...
            """, (sf.credlog[0], sf.credlog[1]))

            user_info = sf.cursor.fetchone()

            if user_info:
                sf.real_name = user_info[0]
                sf.email = user_info[1]
                sf.mobile = user_info[2]

                messagebox.showinfo("Login", f"You have Successfully Logged In\nWelcome {sf.real_name}")
                sf.delivery()
            else:
...
#------address page 13------
    def Address(sf,x):
        ...
        sf.order_button = Button(
        sf.add_frame2,
        text="Order Now",
        command=lambda: sf.save_order_to_db(
            sf.city.get(),
            sf.local.get(),
            sf.build.get(),
            sf.house.get(),
...
#-----order page 14------
    ...
    def Orderde(sf,x):
        ...
        if sf.x=="pick":
            sf.y=sf.orderpay
        sf.pay=Button(sf.order_f2,text="Pay",command=lambda:sf.pay_page(sf.x),bg="#0b1335",cursor="hand2",fg="white",bd=5,font=("default",16,'bold'))
        ...
        #sweet_hours.save_order_to_db(sf)
        sf.scr.mainloop()
